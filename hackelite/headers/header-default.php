...
?>
<div class="header header-default header-style-default v1 <?php echo esc_attr(implode(' ', $header_class)); ?> ">
...
<?php } ?>
    ...
    <?php } elseif (is_user_logged_in() && current_user_can('apply_job')) { ?>
        ...
        <?php if ($show_post_a_job && function_exists('iwj_get_page_permalink')) { ?>
                    ...
                    ?>
                    <?php
                    if (class_exists('IWJ_Class') && $show_button_login_user && $show_button_login_user != 'no') {
                        $user = IWJ_User::get_user();
    ...
    ?>
                            <?php } ?>
<?php if (function_exists('WC') && ($cartTotal > 0) && $show_cart_button && $show_cart_button != 'no') { ?>
...
<?php if (class_exists('IWJ_Class') && $show_form_find_jobs == 'yes' && !( is_page(iwj_get_page_id('candidates')) )) { ?>
        ...
        <?php } elseif (class_exists('IWJ_Class') && $show_search_form_jobs && (is_tax(get_object_taxonomies('iwj_job')) || is_singular('iwj_job'))) {
        ?>
